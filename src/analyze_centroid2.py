...
# Paths added after the first release (and rows under their target) draw from a separate stream, so the
...
def main():
    ...
    for p in shifts:
        ...
        if tgt:
            ...
            row["T_lo"], row["T_hi"] = sorted(-np.array(boot(lambda s: sel(shifts[p][tgt], spk[p], s).mean(), rng=prng)))
    ...
    for p in [q for q in ["resynth_vocos", "resynth_hift", "resynth_s3vc"] if q in TARGET] + [q for q in _EXTRA_TARGET if q in TARGET]:
        ...
        def stat_min(s):
            ...
            return gm - gu
        lo3, hi3 = boot(stat_min, n=a.nboot_min, rng=prng)
    ...
    for q in unm + [x for x in ["resynth_bigvgan"] if x in shifts]:
        for tgt in sorted(set(v for v in TARGET.values() if v)):
            ...
            trng = RNG_ADDED if tgt in ADDED_TARGETS else RNG
            tlo, thi = sorted(-np.array(boot(lambda s: sel(shifts[q][tgt], spk[q], s).mean(), rng=trng)))
            glo, ghi = sorted(-np.array(boot(lambda s: sel(ct, spk[q], s).mean(), rng=trng)))
