...
python src/analyze_primary_knn_sensitivity.py                  # §4, Table 1 tie-rule sensitivity of Table 1 (post hoc, 2026-09-22)
python src/analyze_mfcc_cloning.py                              # Table 1 (MFCC baseline)
python src/analyze_simplecues.py                                # Table 1 (prosodic baseline)
python src/analyze_asr.py                                       # Table 1 (Whisper-error baseline), §4.2 WER=0 control
...
python src/analyze_centroid2.py --ssl w2vbert --layer 19        # §4.1 w2v-BERT L19 statement
python src/analyze_decoder_swap.py --ssl wavlm --layer 0        # §4.1 same-mel decoder swap (D_G) + cross-mel GL table
python src/analyze_lineage.py --ssl wavlm --layer 0             # §4.1 CosyVoice3/Chatterbox own-vs-related alignment
python src/analyze_deconly.py --ssl wavlm                       # §4.1 decoder-only control
python src/analyze_deep_probe.py                                # §4.2 logistic / centroid / kNN deep-band probes
python src/analyze_knn_mechanism.py                             # §4.2 paired-kNN mechanism
python src/analyze_robustB3.py                                  # Table 4 (WavLM) + w2v-BERT
python src/analyze_normalization_control.py                     # §4 trim+RMS control
# §4.1 nuisance control on the interventions (control N2): needs the trim+RMS embeddings,
...
if [ -f results/embeddings/wavlm_tn/resynth_vocos.npz ]; then
  python src/analyze_transplant2.py --ssl wavlm_tn --layers 0 2 4 8 19   # §4.1 normalized target shifts
  python src/analyze_centroid2.py   --ssl wavlm_tn --layer 0             # §4.1 normalized G_adj, G_min
  python src/analyze_decoder_swap.py --ssl wavlm_tn --layer 0            # decoder swap under trim+RMS
fi
python src/analyze_seeds2.py                                    # §4 controlled-seed study
